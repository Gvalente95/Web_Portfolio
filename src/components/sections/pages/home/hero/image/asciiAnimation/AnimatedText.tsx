...
export function useTextEffect({ target, start = "", delay = 0, duration = 2000, stepDuration = 0, type = "random" }: TextEffectData) {
  ...
  }, [target, delay, duration, stepDuration, type]);

  return { text };
}

function TextBlock(props: TextEffectData & { color?: string }) {
  const { text } = useTextEffect(props);
...
export function AnimatedText({ segments, delay = 0, duration, durationPerChar = 60, stepDuration = 0 }: AnimatedTextProps) {
  let accumulatedDelay = delay;

  return (
    <>
      {segments.map((seg, i) => {
        const segDuration = seg.duration ?? duration ?? seg.text.length * durationPerChar;

        const segDelay = accumulatedDelay;

        accumulatedDelay += segDuration + stepDuration * seg.text.length;

        return <TextBlock key={i} target={seg.text} type={seg.type} color={seg.color} delay={segDelay} duration={segDuration} stepDuration={stepDuration} />;
      })}
    </>
  );
}
